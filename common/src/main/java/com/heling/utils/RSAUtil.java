    ...
    public static String encryptByprivateKey(String content, String privateKeyStr, int opmode) {
        ...
        try {
            ...
            if (opmode == Cipher.ENCRYPT_MODE) { // 加密
                ...
                out.close();
                text = Base64.encodeBase64String(encryptedData);
            } else if (opmode == Cipher.DECRYPT_MODE) { // 解密
                while (inputLen - offSet > 0) {
                    if (inputLen - offSet > MAX_DECRYPT_BLOCK) {
                        cache = cipher.doFinal(content.getBytes(), offSet, MAX_DECRYPT_BLOCK);
                    } else {
                        cache = cipher.doFinal(content.getBytes(), offSet, inputLen - offSet);
                    }
                    out.write(cache, 0, cache.length);
                ...
                out.close();
                text = new String(decryptedData, "UTF-8");
            }
        } catch (Exception e) {
            // TODO Auto-generated catch block
            e.printStackTrace();
...
//        // 公钥加密
        ...
        String s = encryptByPublicKey(content, RSAUtil.publicKey, Cipher.ENCRYPT_MODE);

        String s1 = encryptByprivateKey(s, RSAUtil.privateKey, Cipher.DECRYPT_MODE);
...
//        String request = encryptByPublicKey(JSONObject.toJSONString(map), publicKeyA, Cipher.ENCRYPT_MODE);
//
...
//        //平台接收请求进行解密验签
//        //使用私钥A解密
//        String s = encryptByprivateKey(request, privateKeyA, Cipher.DECRYPT_MODE);
